/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package enchantedcs;

import java.io.BufferedReader;
import java.io.DataInputStream;
import java.io.FileInputStream;
import java.io.InputStreamReader;
import java.util.ArrayList;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 *
 * @author Hexa Null
 */
public class EnchantedCS {

    List<String> listKata;

    public EnchantedCS() {
        listKata = read("katadasar.txt");
    }

    public boolean cekKamus(String kata) {
        // cari di database	
        kata = kata.toLowerCase();
        return listKata.contains(kata);

    }

// Hapus Inflection Suffixes (“-lah”, “-kah”, “-ku”, “-mu”, atau “-nya”)
    public String Del_Inflection_Suffixes(String kata) {
        String kataAsal = kata;
        if (preg_match("([km]u|nya|[kl]ah|pun)$", kata)) { // Cek Inflection Suffixes

            String __kata = preg_replace("([km]u|nya|[kl]ah|pun)$", "", kata);

            if (preg_match("([klt]ah|pun)$", kata)) { // Jika berupa particles (“-lah”, “-kah”, “-tah” atau “-pun”)
                if (preg_match("([km]u|nya)$", __kata)) { // Hapus Possesive Pronouns (“-ku”, “-mu”, atau “-nya”)
                    String __kata__ = preg_replace("([km]u|nya)$", "", __kata);

                    return __kata__;

                }
            }
            return __kata;
        }
        return kataAsal;
    }

    public boolean Cek_Rule_Precedence(String kata) {
        if (preg_match("^(be)\\p{Alpha}+(lah|an)$", kata)) { // be- dan -i
            return true;
        }
        if (preg_match("^(di|([mpt]e))\\p{Alpha}+(i)$", kata)) { // di- dan -an				
            return true;
        }
        return false;
    }

// Cek Prefix Disallowed Sufixes (Kombinasi Awalan dan Akhiran yang tidak diizinkan)
    public boolean Cek_Prefix_Disallowed_Sufixes(String kata) {
        if (preg_match("^(be)\\p{Alpha}+(i)$", kata)) { // be- dan -i
            return true;
        }
        if (preg_match("^(di)\\p{Alpha}+(an)$", kata)) { // di- dan -an				
            return true;

        }
        if (preg_match("^(ke)\\p{Alpha}+(i|kan)$", kata)) { // ke- dan -i,-kan
            return true;
        }
        if (preg_match("^(me)\\p{Alpha}+(an)$", kata)) { // me- dan -an
            return true;
        }
        if (preg_match("^(se)\\p{Alpha}+(i|kan)$", kata)) { // se- dan -i,-kan
            return true;
        }
        return false;
    }

// Hapus Derivation Suffixes (“-i”, “-an” atau “-kan”)
    public String Del_Derivation_Suffixes(String kata) {
        String kataAsal = kata;
        if (preg_match("(kan)$", kata)) { // Cek Suffixes
            String __kata = preg_replace("(kan)$", "", kata);
            if (cekKamus(__kata)) { // Cek Kamus			
                return __kata;
            }
        }
        if (preg_match("(an|i)$", kata)) { // cek -kan 				
            String __kata__ = preg_replace("(an|i)$", "", kata);
          
            if (cekKamus(__kata__)) { // Cek Kamus
               
                return __kata__;
            }
        }
        if (Cek_Prefix_Disallowed_Sufixes(kata)) {
            return kataAsal;
        }
        return kataAsal;
    }

// Hapus Derivation Prefix (“di-”, “ke-”, “se-”, “te-”, “be-”, “me-”, atau “pe-”)
    public String Del_Derivation_Prefix(String kata) {
        String kataAsal = kata;
        /*
         * ------ Tentukan Tipe Awalan ------------
         */
        if (preg_match("^(di|[ks]e)\\S{1,}", kata)) { // Jika di-,ke-,se-
            String __kata = preg_replace("^(di|[ks]e)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
         
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        if (preg_match("^([^aiueo])e\\1[aiueo]\\S{1,}/i", kata)) { // aturan  37
            String __kata = preg_replace("^([^aiueo])e", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        if (preg_match("^([tmbp]e)\\S{1,}", kata)) { //Jika awalannya adalah “te-”, “me-”, “be-”, atau “pe-”
		/*
             * ------------ Awalan “be-”, ---------------------------------------------
             */
            if (preg_match("^(be)\\S{1,}", kata)) { // Jika awalan “be-”,
                if (preg_match("^(ber)[aiueo]\\S{1,}", kata)) { // aturan 1.
                    String __kata = preg_replace("^(ber)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(ber)", "r", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(ber)[^aiueor]\\p{Alpha}(?!er)\\S{1,}", kata)) { //aturan  2.
                    String __kata = preg_replace("^(ber)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(ber)[^aiueor]\\p{Alpha}er[aiueo]\\S{1,}", kata)) { //aturan  3.
                    String __kata = preg_replace("^(ber)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^belajar\\S{0,}", kata)) { //aturan  4.
                    String __kata = preg_replace("^(bel)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(be)[^aiueolr]er[^aiueo]\\S{1,}", kata)) { //aturan  5.
                    String __kata = preg_replace("^(be)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
            }
            /*
             * ------------end “be-”, ---------------------------------------------
             */
            /*
             * ------------ Awalan “te-”, ---------------------------------------------
             */
            if (preg_match("^(te)\\S{1,}", kata)) { // Jika awalan “te-”,

                if (preg_match("^(terr)\\S{1,}", kata)) {
                    return kata;
                }
                if (preg_match("^(ter)[aiueo]\\S{1,}", kata)) { // aturan 6.
                    String __kata = preg_replace("^(ter)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(ter)", "r", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(ter)[^aiueor]er[aiueo]\\S{1,}", kata)) { // aturan 7.
                    String __kata = preg_replace("^(ter)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
                if (preg_match("^(ter)[^aiueor](?!er)\\S{1,}", kata)) { // aturan 8.
                    String __kata = preg_replace("^(ter)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
                if (preg_match("^(te)[^aiueor]er[aiueo]\\S{1,}", kata)) { // aturan 9.
                    String __kata = preg_replace("^(te)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(ter)[^aiueor]er[^aiueo]\\S{1,}", kata)) { // aturan  35 belum bisa
                    String __kata = preg_replace("^(ter)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
            }
            /*
             * ------------end “te-”, ---------------------------------------------
             */
            /*
             * ------------ Awalan “me-”, ---------------------------------------------
             */
            if (preg_match("^(me)\\S{1,}", kata)) { // Jika awalan “me-”,

                if (preg_match("^(me)[lrwyv][aiueo]", kata)) { // aturan 10
                    String __kata = preg_replace("^(me)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(mem)[bfvp]\\S{1,}", kata)) { // aturan 11.
                    String __kata = preg_replace("^(mem)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
                /*
                 * if(preg_match("^(mempe)\\S{1,}",kata)){ // aturan 12 __kata =
                 * preg_replace("^(mem)","pe",kata);	                 *
                 * if(cekKamus(__kata)){
                 *
                 * return __kata; // Jika ada balik } __kata__ =
                 * Del_Derivation_Suffixes(__kata); if(cekKamus(__kata__)){
                 * return __kata__; }				
			}
                 */
                if (preg_match("^(mem)((r[aiueo])|[aiueo])\\S{1,}", kata)) {//aturan 13
                    String __kata = preg_replace("^(mem)", "m", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(mem)", "p", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(men)[cdjszt]\\S{1,}", kata)) { // aturan 14.
                    String __kata = preg_replace("^(men)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(men)[aiueo]\\S{1,}", kata)) {//aturan 15
                    String __kata = preg_replace("^(men)", "n", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(men)", "t", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(meng)[ghqk]\\S{1,}", kata)) { // aturan 16.
                    String __kata = preg_replace("^(meng)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(meng)[aiueo]\\S{1,}", kata)) { // aturan 17
                    String __kata = preg_replace("^(meng)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(meng)", "k", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(menge)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(meny)[aiueo]\\S{1,}", kata)) { // aturan 18.
                    String __kata = preg_replace("^(meny)", "s", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(me)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
            }
            /*
             * ------------end “me-”, ---------------------------------------------
             */

            /*
             * ------------ Awalan “pe-”, ---------------------------------------------
             */
            if (preg_match("^(pe)\\S{1,}", kata)) { // Jika awalan “pe-”,

                if (preg_match("^(pe)[wy]\\S{1,}", kata)) { // aturan 20.
                    String __kata = preg_replace("^(pe)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(per)[aiueo]\\S{1,}", kata)) { // aturan 21
                    String __kata = preg_replace("^(per)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(per)", "r", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }
                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
                if (preg_match("^(per)[^aiueor]\\p{Alpha}(?!er)\\S{1,}", kata)) { // aturan  23
                    String __kata = preg_replace("^(per)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(per)[^aiueor]\\p{Alpha}(er)[aiueo]\\S{1,}", kata)) { // aturan  24
                    String __kata = preg_replace("^(per)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pem)[bfv]\\S{1,}", kata)) { // aturan  25
                    String __kata = preg_replace("^(pem)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pem)(r[aiueo]|[aiueo])\\S{1,}", kata)) { // aturan  26
                    String __kata = preg_replace("^(pem)", "m", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(pem)", "p", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pen)[cdjzt]\\S{1,}", kata)) { // aturan  27
                    String __kata = preg_replace("^(pen)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pen)[aiueo]\\S{1,}", kata)) { // aturan  28
                    String __kata = preg_replace("^(pen)", "n", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(pen)", "t", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(peng)[^aiueo]\\S{1,}", kata)) { // aturan  29
                    String __kata = preg_replace("^(peng)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(peng)[aiueo]\\S{1,}", kata)) { // aturan  30
                    String __kata = preg_replace("^(peng)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(peng)", "k", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(penge)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(peny)[aiueo]\\S{1,}", kata)) { // aturan  31
                    String __kata = preg_replace("^(peny)", "s", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                    __kata = preg_replace("^(pe)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pel)[aiueo]\\S{1,}", kata)) { // aturan  32
                    String __kata = preg_replace("^(pel)", "l", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pelajar)\\S{0,}", kata)) {
                    String __kata = preg_replace("^(pel)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pe)[^rwylmn]er[aiueo]\\S{1,}", kata)) { // aturan  33
                    String __kata = preg_replace("^(pe)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pe)[^rwylmn](?!er)\\S{1,}", kata)) { // aturan  34
                    String __kata = preg_replace("^(pe)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }

                if (preg_match("^(pe)[^aiueor]er[^aiueo]\\S{1,}", kata)) { // aturan  36
                    String __kata = preg_replace("^(pe)", "", kata);
                    if (cekKamus(__kata)) {
                        return __kata; // Jika ada balik
                    }

                    String __kata__ = Del_Derivation_Suffixes(__kata);
                    if (cekKamus(__kata__)) {
                        return __kata__;
                    }
                }
            }
        }
        /*
         * ------------end “pe-”, ---------------------------------------------
         */
        /*
         * ------------ Awalan “memper-”, ---------------------------------------------
         */
        if (preg_match("^(memper)\\S{1,}", kata)) {
            String __kata = preg_replace("^(memper)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            //*-- Cek luluh -r ----------
            __kata = preg_replace("^(memper)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “memper-”, ---------------------------------------------
         */
        /*
         * ------------ Awalan “mempel-”, ---------------------------------------------
         */
        if (preg_match("^(mempel)\\S{1,}", kata)) {
            String __kata = preg_replace("^(mempel)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            //*-- Cek luluh -r ----------
            __kata = preg_replace("^(mempel)", "l", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “mempel-”, ---------------------------------------------
         */
        /*
         * ------------awalan “memter-”, ---------------------------------------------
         */
        if (preg_match("^(menter)\\S{1,}", kata)) {
            String __kata = preg_replace("^(menter)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            //*-- Cek luluh -r ----------
            __kata = preg_replace("^(menter)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “memter-”, ---------------------------------------------
         */
        /*
         * ------------awalan “member-”, ---------------------------------------------
         */
        if (preg_match("^(member)\\S{1,}", kata)) {
            String __kata = preg_replace("^(member)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            //*-- Cek luluh -r ----------
            __kata = preg_replace("^(member)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end member-”, ---------------------------------------------
         */
        /*
         * ------------awalan “diper-”, ---------------------------------------------
         */
        if (preg_match("^(diper)\\S{1,}", kata)) {
            String __kata = preg_replace("^(diper)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -r ----------
             */
            __kata = preg_replace("^(diper)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “diper-”, ---------------------------------------------
         */
        /*
         * ------------awalan “diter-”, ---------------------------------------------
         */
        if (preg_match("^(diter)\\S{1,}", kata)) {
            String __kata = preg_replace("^(diter)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -r ----------
             */
            __kata = preg_replace("^(diter)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “diter-”, ---------------------------------------------
         */
        /*
         * ------------awalan “dipel-”, ---------------------------------------------
         */
        if (preg_match("^(dipel)\\S{1,}", kata)) {
            String __kata = preg_replace("^(dipel)", "l", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -l----------
             */
            __kata = preg_replace("^(dipel)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end dipel-”, ---------------------------------------------
         */
        /*
         * ------------kata “terpelajar”(kasus khusus), ---------------------------------------------
         */
        if (preg_match("terpelajar", kata)) {
            String __kata = preg_replace("terpel", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end “terpelajar”-”, ---------------------------------------------
         */
        /*
         * ------------kata seseorang(kasus khusus), ---------------------------------------------
         */
        if (preg_match("seseorang", kata)) {
            String __kata = preg_replace("^(sese)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
        }
        /*
         * ------------end seseorang-”, ---------------------------------------------
         */
        /*
         * ------------awalan "diber-"---------------------------------------------
         */
        if (preg_match("^(diber)\\S{1,}", kata)) {
            String __kata = preg_replace("^(diber)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -l----------
             */
            __kata = preg_replace("^(diber)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end "diber-"---------------------------------------------
         */
        /*
         * ------------awalan "keber-"---------------------------------------------
         */
        if (preg_match("^(keber)\\S{1,}", kata)) {
            String __kata = preg_replace("^(keber)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -l----------
             */
            __kata = preg_replace("^(keber)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end "keber-"---------------------------------------------
         */
        /*
         * ------------awalan "keter-"---------------------------------------------
         */
        if (preg_match("^(keter)\\S{1,}", kata)) {
            String __kata = preg_replace("^(keter)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
            /*
             * -- Cek luluh -l----------
             */
            __kata = preg_replace("^(keter)", "r", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end "keter-"---------------------------------------------
         */
        /*
         * ------------awalan "berke-"---------------------------------------------
         */
        if (preg_match("^(berke)\\S{1,}", kata)) {
            String __kata = preg_replace("^(berke)", "", kata);
            if (cekKamus(__kata)) {
                return __kata; // Jika ada balik
            }
            String __kata__ = Del_Derivation_Suffixes(__kata);
            if (cekKamus(__kata__)) {
                return __kata__;
            }
        }
        /*
         * ------------end "berke-"---------------------------------------------
         */
        /*
         * --- Cek Ada Tidaknya Prefik/Awalan (“di-”, “ke-”, “se-”, “te-”,
         * “be-”, “me-”, atau “pe-”) ------
         */
        if (preg_match("^(di|[kstbmp]e)\\S{1,}", kata) == false) {
            return kataAsal;
        }

        return kataAsal;
    }

    public String Enhanced_CS(String kata) {

        String kataAsal = kata;

        /*
         * 1. Cek Kata di Kamus jika Ada SELESAI
         */
        if (cekKamus(kata)) { // Cek Kamus

            return kata; // Jika Ada kembalikan
        }
        /*
         * 2. Buang Infection suffixes (\-lah", \-kah", \-ku", \-mu", atau
         * \-nya")
         */
        kata = Del_Inflection_Suffixes(kata);

        /*
         * 3. Buang Derivation suffix (\-i" or \-an")
         */
        kata = Del_Derivation_Suffixes(kata);

        /*
         * 4. Buang Derivation prefix
         */
        kata = Del_Derivation_Prefix(kata);

        return kata;
    }

    public static boolean preg_match(String pttr, String input) {
        Pattern p = Pattern.compile(pttr);
        Matcher m = p.matcher(input);
        return m.find();
    }

    public static String preg_replace(String pate, String repla, String kata) {
        return kata.replaceAll(pate, repla);
    }

    public List<String> read(String name) {
        List<String> ls = new ArrayList<String>();
        //MEMBACA FILE
        try {
// Open the file that is the first
// command line parameter
            FileInputStream fstream = new FileInputStream(name);
// Get the object of DataInputStream
            DataInputStream in = new DataInputStream(fstream);
            BufferedReader br = new BufferedReader(new InputStreamReader(in));

            String strLine;
//Read File Line By Line
            while ((strLine = br.readLine()) != null) {
// Print the content on the console
                //    System.out.println(strLine);

                ls.add(strLine.trim().toLowerCase());
            }
//Close the input stream
            in.close();
        } catch (Exception e) {//Catch exception if any
            System.err.println("Error: " + e.getMessage());
        }
        return ls;
    }
//public static boolean eregi(String pttr, String input) {
//        Pattern p = Pattern.compile(pttr);
//        Matcher m = p.matcher(input);
//        return m.find();
//    }
//
//public static  String eregi_replace(String pate,String repla,String kata){
//return  kata.replaceAll(pate, repla);
//}

    public static void main(String[] args) {
        System.out.println(new EnchantedCS().Enhanced_CS("kesatuan"));
    }
}
